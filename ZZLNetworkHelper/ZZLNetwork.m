...
#ifdef DEBUG
#define ZZLLog(FORMAT, ...) fprintf(stderr,"[%s:%d行] %s\n",[[[NSString stringWithUTF8String:__FILE__] lastPathComponent] UTF8String], __LINE__, [[NSString stringWithFormat:FORMAT, ##__VA_ARGS__] UTF8String]);
#else
#define ZZLLog(...)
#endif


@implementation ZZLNetwork
...
static NSString *const NetworkResponseCache = @"ZZLNetworkResponseCache";
...
/*实时获取网络状态*/
+ (void)getNetworkStatusWithBlock:(ZZLNetworkStatus)networkStatus{
        ...
        [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
            switch (status) {
                case AFNetworkReachabilityStatusUnknown:
                    networkStatus ? networkStatus(ZZLNetworkStatusUnknown) : nil;
                    break;
                case AFNetworkReachabilityStatusNotReachable:
                    networkStatus ? networkStatus(ZZLNetworkStatusNotReachable) : nil;
                    break;
                case AFNetworkReachabilityStatusReachableViaWWAN:
                    networkStatus ? networkStatus(ZZLNetworkStatusReachableWWAN) : nil;
                    break;
                case AFNetworkReachabilityStatusReachableViaWiFi:
                    networkStatus ? networkStatus(ZZLNetworkStatusReachableWiFi) : nil;
                    break;
                default:
                    break;
            }
...
#pragma mark -- 缓存描述文字
+ (NSString *)cachePolicyStr:(ZZLCachePolicy)cachePolicy
{
    switch (cachePolicy) {
        case ZZLCachePolicyIgnoreCache:
            return @"只从网络获取数据，且数据不会缓存在本地";
            break;
        case ZZLCachePolicyCacheOnly:
            return @"只从缓存读数据，如果缓存没有数据，返回一个空";
            break;
        case ZZLCachePolicyNetworkOnly:
            return @"先从网络获取数据，同时会在本地缓存数据";
            break;
        case ZZLCachePolicyCacheElseNetwork:
            return @"先从缓存读取数据，如果没有再从网络获取";
            break;
        case ZZLCachePolicyNetworkElseCache:
            return @"先从网络获取数据，如果没有再从缓存读取数据";
            break;
        case ZZLCachePolicyCacheThenNetwork:
            return @"先从缓存读取数据，然后再从网络获取数据，Block将产生两次调用";
            break;
            
        default:
            return @"未知缓存策略，采用ZZLCachePolicyIgnoreCache策略";
            break;
    }
}
...
+ (void)GETWithURL:(NSString *)url
       ...
       cachePolicy:(ZZLCachePolicy)cachePolicy
          callback:(ZZLHttpRequest)callback
{
    [self HTTPWithMethod:ZZLRequestMethodGET url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)POSTWithURL:(NSString *)url
        ...
        cachePolicy:(ZZLCachePolicy)cachePolicy
           callback:(ZZLHttpRequest)callback
{
    [self HTTPWithMethod:ZZLRequestMethodPOST url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)HEADWithURL:(NSString *)url
        ...
        cachePolicy:(ZZLCachePolicy)cachePolicy
           callback:(ZZLHttpRequest)callback
{
    [self HTTPWithMethod:ZZLRequestMethodHEAD url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)PUTWithURL:(NSString *)url
       ...
       cachePolicy:(ZZLCachePolicy)cachePolicy
          callback:(ZZLHttpRequest)callback
{
    [self HTTPWithMethod:ZZLRequestMethodPUT url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)PATCHWithURL:(NSString *)url
         ...
         cachePolicy:(ZZLCachePolicy)cachePolicy
            callback:(ZZLHttpRequest)callback
{
    [self HTTPWithMethod:ZZLRequestMethodPATCH url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)DELETEWithURL:(NSString *)url
          ...
          cachePolicy:(ZZLCachePolicy)cachePolicy
             callback:(ZZLHttpRequest)callback
{
    [self HTTPWithMethod:ZZLRequestMethodDELETE url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}


+ (void)HTTPWithMethod:(ZZLRequestMethod)method
                   url:(NSString *)url
            parameters:(NSDictionary *)parameters
           cachePolicy:(ZZLCachePolicy)cachePolicy
              callback:(ZZLHttpRequest)callback
{
    ...
    if (cachePolicy == ZZLCachePolicyIgnoreCache) {
        //只从网络获取数据，且数据不会缓存在本地
        [self httpWithMethod:method url:url parameters:parameters callback:callback];
    }else if (cachePolicy == ZZLCachePolicyCacheOnly){
    ...
    }else if (cachePolicy == ZZLCachePolicyNetworkOnly){
        //先从网络获取数据，同时会在本地缓存数据
    ...
    }else if (cachePolicy == ZZLCachePolicyCacheElseNetwork){
        //先从缓存读取数据，如果没有再从网络获取
        [self httpCacheForURL:url parameters:parameters withBlock:^(id<NSCoding> object) {
            if (object) {
                callback ? callback(object, YES, nil) : nil;
    ...
    }else if (cachePolicy == ZZLCachePolicyNetworkElseCache){
    ...
    }else if (cachePolicy == ZZLCachePolicyCacheThenNetwork){
        ...
        //缓存策略错误，将采取 ZZLCachePolicyIgnoreCache 策略
        ZZLLog(@"缓存策略错误");
        [self httpWithMethod:method url:url parameters:parameters callback:callback];
    }
...
#pragma mark -- 网络请求处理
+(void)httpWithMethod:(ZZLRequestMethod)method url:(NSString *)url parameters:(NSDictionary *)parameters callback:(ZZLHttpRequest)callback{
    ...
    [self dataTaskWithHTTPMethod:method url:url parameters:parameters callback:^(NSURLSessionDataTask * _Nonnull task, id _Nullable responseObject) {
        if (_logEnabled) {
            ZZLLog(@"请求结果 = %@",[self jsonToString:responseObject]);
        }
        [[self allSessionTask] removeObject:task];
...
#pragma mark -- 上传文件
+ (void)uploadFileWithURL:(NSString *)url parameters:(NSDictionary *)parameters name:(NSString *)name filePath:(NSString *)filePath progress:(ZZLHttpProgress)progress callback:(ZZLHttpRequest)callback
{
...
#pragma mark -- 上传图片文件
+ (void)uploadImageURL:(NSString *)url parameters:(NSDictionary *)parameters images:(NSArray<UIImage *> *)images name:(NSString *)name fileName:(NSString *)fileName mimeType:(NSString *)mimeType progress:(ZZLHttpProgress)progress callback:(ZZLHttpRequest)callback;
{
    NSURLSessionTask *sessionTask = [_sessionManager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //压缩-添加-上传图片
...
#pragma mark -- 下载文件
+(void)downloadWithURL:(NSString *)url fileDir:(NSString *)fileDir progress:(ZZLHttpProgress)progress callback:(ZZLHttpDownload)callback
{
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:url]];
    __block NSURLSessionDownloadTask *downloadTask = [_sessionManager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
        if (_logEnabled) {
            ZZLLog(@"下载进度:%.2f%%",100.0*downloadProgress.completedUnitCount / downloadProgress.totalUnitCount);
        }
...
+ (NSString *)getMethodStr:(ZZLRequestMethod)method{
    switch (method) {
        case ZZLRequestMethodGET:
            return @"GET";
            break;
        case ZZLRequestMethodPOST:
            return @"POST";
            break;
        case ZZLRequestMethodHEAD:
            return @"HEAD";
            break;
        case ZZLRequestMethodPUT:
            return @"PUT";
            break;
        case ZZLRequestMethodPATCH:
            return @"PATCH";
            break;
        case ZZLRequestMethodDELETE:
            return @"DELETE";
            break;
...
+ (void)httpCacheForURL:(NSString *)url parameters:(NSDictionary *)parameters withBlock:(void(^)(id responseObject))block
{
    ...
    [_dataCache objectForKey:cacheKey withBlock:^(NSString * _Nonnull key, id<NSCoding>  _Nonnull object) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (_logEnabled) {
                ZZLLog(@"缓存结果 = %@",[self jsonToString:object]);
            }
            block(object);
        });
    }];
...
+ (void)setRequestSerializer:(ZZLRequestSerializer)requestSerializer{
    _sessionManager.requestSerializer = requestSerializer==ZZLRequestSerializerHTTP ? [AFHTTPRequestSerializer serializer] : [AFJSONRequestSerializer serializer];
}

+ (void)setResponseSerializer:(ZZLResponseSerializer)responseSerializer{
    _sessionManager.responseSerializer = responseSerializer==ZZLResponseSerializerHTTP ? [AFHTTPResponseSerializer serializer] : [AFJSONResponseSerializer serializer];
}
...
@implementation NSDictionary (ZZL)
